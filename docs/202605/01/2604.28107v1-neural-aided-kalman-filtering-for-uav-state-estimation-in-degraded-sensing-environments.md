...
tldr: 针对无人机在复杂环境下的状态估计难题，本文提出了一种结合贝叶斯神经网络（BNN）与卡尔曼滤波的混合框架BNKF。该方法利用BNN学习复杂的非线性运动规律并量化不确定性，将其直接融入卡尔曼滤波的协方差传播中。实验表明，在传感器噪声大、采样率低的情况下，BNKF在精度和鲁棒性上均优于传统的EKF和UKF，且具备实时部署能力。
selection_source: fresh_fetch
motivation: 传统卡尔曼滤波在处理具有敏捷非线性运动和高噪声传感数据的无人机状态估计时，因无法有效处理未知控制输入和不确定性而表现不佳。
method: 提出BNKF框架，利用贝叶斯神经网络预测状态并量化不确定性，随后通过卡尔曼修正步骤实现鲁棒的在线状态估计。
result: 在合成无人机飞行数据上的测试显示，BNKF在准确性、精度和真值包含率方面均显著优于扩展卡尔曼滤波（EKF）和无迹卡尔曼滤波（UKF）。
conclusion: BNKF通过融合深度学习的非线性建模能力与卡尔曼滤波的修正机制，为退化传感环境下的无人机实时状态估计提供了一种高效且鲁棒的解决方案。
---

## 摘要
非线性动力系统的准确状态估计是现代空、海、天领域航空航天操作的基础。由于敏捷的非线性运动、噪声大且稀疏的传感器测量以及未知的控制输入，对抗性无人机（UAV）的在线跟踪尤其具有挑战性；这些条件违反了经典卡尔曼滤波变体的关键假设，并降低了估计性能。神经网络（NN）可以从数据中学习复杂的非线性关系，但缺乏原则性的不确定性量化，而这对于置信区间驱动下游决策的状态估计任务至关重要。我们利用贝叶斯神经网络（BNN）来解决这一问题，BNN 通过网络权重的分布对不确定性进行建模，并通过蒙特卡洛采样产生预测均值和不确定性。在此基础上，我们提出了贝叶斯神经卡尔曼滤波器（BNKF）：一种将训练好的 BNN 与卡尔曼修正步骤相结合的混合框架，用于鲁棒的在线无人机状态估计。与相关的神经卡尔曼方法不同，BNKF 生成完整的状态预测，并将贝叶斯不确定性直接纳入协方差传播中，从而提高了高噪声条件下的鲁棒性。我们使用合成的非线性无人机飞行数据，在不同的雷达噪声水平和采样率下评估了 BNKF。五折交叉验证表明，在降级传感条件下，BNKF 在准确度、精密度和真值包含率方面优于扩展卡尔曼滤波器（EKF）和无迹卡尔曼滤波器（UKF）。集成变体（BNKFe）在牺牲少量准确度的前提下，进一步提高了高噪声极端情况下的精密度。运行时间分析确认了极小的推理开销，支持实时部署的可行性。
