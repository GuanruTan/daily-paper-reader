...
tldr: 针对云端部署 VLA 模型在机器人导航中因网络延迟导致的空时错位问题，本文提出 AsyncShield 框架。该框架通过物理空间映射将时间延迟转化为位姿偏移，并利用强化学习适配器在追踪 VLA 意图与避障硬约束之间取得平衡。作为轻量级即插即用模块，它在无需微调大模型的情况下，显著提升了异步导航的成功率与安全性。
selection_source: fresh_fetch
motivation: 云端部署 VLA 模型带来的网络抖动和推理延迟会导致机器人导航中的指令失效，进而引发碰撞风险。
method: 提出一种基于物理白盒空间映射的异步控制框架，结合时空位姿缓存与强化学习适配器，动态平衡意图恢复与避障约束。
result: 仿真和实机实验表明，该框架在不微调云端基础模型的情况下，展现出强大的零样本泛化能力和鲁棒性。
conclusion: AsyncShield 为解决云端大模型在实时机器人控制中的延迟挑战提供了一种高效、安全且易于部署的通用方案。
---

## 摘要
虽然视觉-语言-动作（VLA）模型已被证明在机器人控制方面具有强大的零样本泛化能力，但其庞大的参数规模通常需要云端部署。然而，云端部署会引入网络抖动和推理延迟，这在连续位移下的移动导航中可能导致严重的时空错位，使得过去本体帧中表达的陈旧意图在当前帧中可能变得空间错误，从而导致碰撞。为了解决这一问题，我们提出了 AsyncShield，一个即插即用的异步控制框架。AsyncShield 摒弃了传统的黑盒时间序列预测，转而采用确定性的物理白盒空间映射。通过维护时间位姿缓存并利用运动学变换，该系统能够准确地将时间滞后转换为空间位姿偏移，从而恢复 VLA 原始的几何意图。为了平衡意图恢复的保真度与物理安全性，边缘适配被建模为一个受约束的马尔可夫决策过程（CMDP）。通过 PPO-Lagrangian 算法求解，强化学习适配器在追踪 VLA 意图与响应高频 LiDAR 避障硬约束之间进行动态权衡。此外，得益于标准化的通用子目标接口、领域随机化以及通过碰撞半径膨胀实现的感知层适配，AsyncShield 作为一个轻量级的即插即用模块运行。仿真和真实世界实验表明，在不微调任何云端基础模型的情况下，该框架展现了零样本且鲁棒的泛化能力，有效提高了异步导航的成功率和物理安全性。
