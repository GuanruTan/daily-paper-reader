...
score: 6.0
evidence: 自主导航中的风场预测
tldr: 本研究提出了一种无人机实时局部风场预测与导航框架。通过融合LiDAR测距与稀疏原位风力测量，利用深度学习和流体力学工具预测城市环境中的复杂风场。该模型被集成到后退水平优化控制器中，以优化路径规划。仿真和风洞实验证明，该方法在无需先验环境知识的情况下，显著降低了无人机的碰撞率和能耗，提升了在强风环境下的自主导航鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决无人机在复杂城市环境中因不可预测的局部风场导致的导航安全与能耗问题。
method: 提出一种融合LiDAR地形数据与稀疏风力测量的深度学习预测模型，并将其先验信息引入后退水平优化控制器。
result: 仿真与实测表明，该系统能实时准确预测风场，显著提升了无人机避障能力并降低了飞行能耗。
conclusion: 该研究为未知风力环境下的自主导航提供了新范式，通过实时风场感知增强了空中机器人在挑战性环境中的生存能力。
---

## 摘要
本论文提出了一种解决方案，使空中机器人能够利用机载传感器和嵌入式飞行硬件实时推理周围的风流场。本研究的核心创新在于将距离测量与稀疏的原位风力测量相结合，以预测周围的流场。我们旨在解决两个基本问题：首先，在密集城市环境中，地形数据对于准确预测风场是否充分；其次，学习到的风场模型在以能效和避障为重点的运动规划中的实用性。借鉴深度学习、流体力学和最优控制工具，我们建立了一个利用导航激光雷达（LiDAR）进行局部风场预测的框架，随后将局部风场模型先验整合到收缩时界最优控制器中，以研究局部风场知识如何影响自主导航过程中的能量消耗和鲁棒性。通过在多种城市风力场景下的仿真演示，我们评估了风场预测器的预测能力，并量化了将局部风场信息整合到运动规划中后，在碰撞率和能量消耗方面对自主城市导航的改进。在露天风洞中进行的缩比自由飞行实验表明，这些算法可以在嵌入式飞行计算机上实时运行，并具有足够的带宽来稳定控制小型空中机器人。从理念上讲，本论文为未知大风环境下的局部风场推断和运动规划贡献了一种新范式。通过使机器人能够在没有先验环境知识的情况下快速评估局部风力状况，本研究加速了空中机器人进入日益具有挑战性的环境。
