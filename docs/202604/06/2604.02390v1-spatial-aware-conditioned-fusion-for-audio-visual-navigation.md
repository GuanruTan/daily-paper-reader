...
## 摘要
视听导航任务要求智能体仅利用视觉观测和声学线索，对持续发声的目标进行定位并导航。然而，现有方法主要依赖简单的特征拼接或后期融合，且缺乏对目标相对位置的显式离散表示，这限制了学习效率和泛化能力。我们提出了空间感知条件融合（SACF）。SACF 首先从视听线索中对目标的相对方向和距离进行离散化，预测其分布，并将其编码为紧凑的描述符，用于策略调节和状态建模。随后，SACF 利用音频嵌入和空间描述符生成通道级缩放和偏置，通过条件线性变换调制视觉特征，从而产生面向目标的融合表示。SACF 以较低的计算开销提高了导航效率，并能很好地泛化到未听过的目标声音。
