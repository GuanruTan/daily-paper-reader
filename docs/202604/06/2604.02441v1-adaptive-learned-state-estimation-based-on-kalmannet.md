...
title_zh: 基于 KalmanNet 的自适应学习状态估计
...
## 摘要
将基于模型的卡尔曼滤波与学习组件相结合的混合状态估计器在模拟数据上展现出良好的前景，但在真实汽车数据上的性能表现仍然不足。在这项工作中，我们提出了自适应多模态 KalmanNet (AM-KNet)，这是针对多传感器自动驾驶场景定制的 KalmanNet 改进版本。AM-KNet 引入了特定传感器的测量模块，使网络能够独立学习雷达、激光雷达和摄像头的不同噪声特性。具有上下文调制的超网络根据目标类型、运动状态和相对姿态对滤波器进行条件化处理，从而实现对多样化交通场景的自适应。我们进一步结合了基于 Joseph 形式的协方差估计分支，并通过估计误差和新权（innovation）的负对数似然损失对其进行监督。一个全面的、按组件划分的损失函数编码了关于传感器可靠性、目标类别、运动状态和测量流一致性的物理先验。AM-KNet 在 nuScenes 和 View-of-Delft 数据集上进行了训练和评估。结果表明，与基础 KalmanNet 相比，该方法提高了估计精度和跟踪稳定性，缩小了在真实汽车数据上与经典贝叶斯滤波器之间的性能差距。
