...
title_zh: 变色龙不会遗忘：基于提示的在线持续学习用于下一活动预测
...
tldr: 针对预测性过程监控（PPM）中因环境动态变化和概念漂移导致的灾难性遗忘问题，本文提出了一种基于提示词的在线持续学习方法CNAPwP。该方法通过改进DualPrompt算法，在预测下一活动时兼顾准确性与适应性。研究还引入了包含循环漂移的新数据集和特定遗忘指标，实验证明CNAPwP在处理循环任务时表现优异，达到了SOTA水平。
selection_source: fresh_fetch
motivation: 现有的过程预测模型通常假设环境静态，难以应对实际业务流程中的概念漂移和灾难性遗忘问题。
method: 提出CNAPwP方法，将DualPrompt算法应用于下一活动预测任务，利用提示词机制在学习新任务的同时保留旧知识。
result: 在三个合成数据集和两个真实数据集上的实验表明，CNAPwP在应对循环漂移场景时优于或等同于五种基准方法。
conclusion: CNAPwP有效缓解了过程预测中的灾难性遗忘，为动态环境下的业务流程监控提供了具有竞争力的解决方案。
---

## 摘要
预测性过程监控 (PPM) 专注于预测未来的过程轨迹，包括下一活动预测。这在过程发生变化或面临不确定性的动态环境中至关重要。然而，当前的框架通常假设环境是静态的，忽略了动态特征和概念漂移 (concept drifts)。这导致了灾难性遗忘 (catastrophic forgetting)，即仅关注新数据分布的训练会对先前学习的数据分布性能产生负面影响。持续学习 (Continual learning) 旨在解决包括缓解灾难性遗忘在内的各项挑战。本文提出了一种名为“基于提示的持续下一活动预测” (CNAPwP) 的新方法，该方法将 DualPrompt 算法应用于下一活动预测，旨在提高准确性和适应性，同时缓解灾难性遗忘。我们引入了具有循环概念漂移的新数据集，以及一种特定于任务的遗忘指标，用于衡量初始出现与后续任务出现之间的预测准确度差距。在代表多种循环漂移设置的三个合成数据集和两个真实世界数据集上进行的广泛测试表明，与五个基准模型相比，CNAPwP 达到了 SOTA 或具有竞争力的结果，证明了其在真实场景中的潜在适用性。我们方法的开源实现、数据集和结果可在以下网址获取：https://github.com/SvStraten/CNAPwP。
