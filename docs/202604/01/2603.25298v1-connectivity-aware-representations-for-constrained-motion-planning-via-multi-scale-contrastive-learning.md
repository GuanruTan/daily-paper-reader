...
tldr: 针对受限运动规划中起始与目标配置可能处于互不连通区域（EMD）的问题，本文提出一种基于多尺度对比学习的连通性感知表示方法。该方法通过多尺度流形学习和聚类生成伪标签，训练模型识别配置空间的连通性，从而在规划前优化配置选择。实验证明，该方法显著提高了复杂约束下的规划成功率并大幅缩短了计算时间。
selection_source: fresh_fetch
motivation: 解决受限运动规划中由于配置空间不连通和机器人冗余性导致的规划失败及效率低下问题。
method: 采用多尺度流形学习结合聚类伪标签的对比学习框架，学习能够区分不同连通区域的潜在空间表示。
result: "在各项操作任务中，该方法相比基准方案实现了1.9倍的成功率提升，并将规划时间减少了57%。"
conclusion: 引入连通性感知表示能有效引导配置选择，是解决复杂约束运动规划中不连通性挑战的有效途径。
---

## 摘要
约束运动规划的目标是在满足特定任务约束的同时，连接起始和目标配置。当配置位于不连通区域（称为本质上相互断开 (EMD) 的分量）时，运动规划会变得低效或不可行。约束进一步将可行空间限制在低维子流形上，而冗余性则引入了额外的复杂性，因为单个末端执行器位姿允许无限多个逆运动学解，这些解可能形成离散的自运动流形。本文通过学习一种连通性感知表示来解决这些挑战，用于在规划前选择起始和目标配置。关节配置通过跨越从局部到全局邻域范围的多尺度流形学习被嵌入到潜空间中，聚类生成的伪标签用于监督对比学习框架。所提出的框架提供了一种连通性感知度量，该度量倾向于在连通区域中选择起始和目标配置，从而避免 EMD 并以更短的规划时间获得更高的成功率。在各种操作任务上的实验表明，与基准方法相比，我们的方法实现了 1.9 倍的成功率提升，并将规划时间缩短至 0.43 倍。
