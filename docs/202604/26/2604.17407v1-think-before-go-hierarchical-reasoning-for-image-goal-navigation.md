...
tags: ["query:tp"]
score: 6.0
evidence: 智能体导航的高层规划与底层执行
tldr: 针对图像目标导航中远距离目标导致智能体盲目游走的问题，本文提出分层推理导航框架（HRNav）。该框架模拟人类认知，将任务分解为高层规划和底层执行：利用视觉语言模型（VLM）生成短期规划指令，再由强化学习策略执行具体动作。此外，引入游走抑制惩罚（WSP）优化训练。实验证明该方法在仿真和现实场景中均表现优异。
selection_source: fresh_fetch
motivation: 传统的端到端导航方法在目标较远时难以提取有效视觉线索，导致智能体在复杂环境中容易盲目游走。
method: 提出HRNav框架，利用视觉语言模型生成高层短期规划指令，并结合带有游走抑制惩罚的强化学习策略进行底层动作执行。
result: 在仿真环境和真实世界场景的广泛实验中，该方法在处理长程导航任务时表现出显著优于现有方法的性能。
conclusion: 通过将高层逻辑推理与底层反应式执行相结合，HRNav有效提升了图像目标导航的效率和成功率。
---

## 摘要
图像目标导航引导智能体在未知环境中前往由图像指定的特定目标位置。现有方法主要通过学习端到端的导航策略来处理这一任务，该策略通过比较目标图像与观测图像的相似性并直接预测动作。然而，当目标较远或位于另一个房间时，此类方法无法提取有效的信息性视觉线索，导致智能体四处徘徊。受人类认知原理（即在复杂任务中，深思熟虑的高层推理引导快速、反应式的执行）的启发，我们提出了分层推理导航（HRNav），这是一个将图像目标导航分解为高层规划和底层执行的框架。在高层规划中，我们在自收集的数据集上训练了一个视觉语言模型，以生成短时程规划，例如智能体应该穿过门还是沿着走廊走。这降低了长时程任务的难度，使其更易于执行部分的处理。在底层执行中，利用在线强化学习策略根据短时程规划来决定动作。我们还设计了一种新颖的徘徊抑制惩罚（WSP），以进一步减少徘徊问题。这些组件共同构成了一个用于图像目标导航的分层框架。在仿真和真实世界环境中的广泛实验证明了我们方法的优越性。
