...
## 摘要
在包含架子、桌子和床等多种家具的复杂室内环境中高效定位目标物体，是移动机器人面临的一项重大挑战。这种困难源于定位误差、有限视野和视觉遮挡等因素。我们将物体搜索任务建模为 3D 环境中具有增长状态空间和混合（连续与离散）动作空间的高维部分可观测马尔可夫决策过程 (POMDP)。基于精心设计的感知模块，本文提出了一种名为增长神经过程过滤 k-中心聚类树 (GNPF-kCT) 的新型在线 POMDP 求解器来解决这一问题。最优动作的选择采用蒙特卡洛树搜索 (MCTS)，其中包含针对增长状态空间的信念树重用、用于过滤无效原始动作的神经过程网络，以及用于高效细化高维动作空间的 k-中心聚类超球体离散化。一种改进的上置信界 (UCB) 算法，通过估计直径单元内的信念差异和动作价值函数来引导 MCTS 的扩展。理论分析验证了该方法的收敛性和性能潜力。为了应对信息或奖励有限的场景，我们还引入了带有栅格世界模型的猜测目标物体，作为提高搜索效率的关键策略。在相同的计算约束和感知系统下，使用 Fetch 和 Stretch 机器人的大量 Gazebo 仿真表明，在物体搜索中，该方法比基于 POMDP 的基准方法和最先进的 (SOTA) 非 POMDP 求解器（特别是基于大语言模型 (LLM) 的方法）具有更快、更可靠的目标定位能力。在办公环境中的真实世界测试证实了该方法的实际适用性。项目主页：https://sites.google.com/view/gnpfkct。
