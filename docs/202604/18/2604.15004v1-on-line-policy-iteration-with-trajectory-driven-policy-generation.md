...
tldr: 本文针对固定初始状态的确定性有限时界最优控制问题，提出一种在线策略迭代方法。该方法通过当前策略生成的轨迹数据来训练下一代策略，实现成本的单调递减。它结合了展开（rollout）与策略迭代的优势，特别适用于离散优化和重复性路径规划等重复性任务。该框架不仅支持多决策者场景，还为利用轨迹数据训练神经网络策略并确保性能单调提升提供了理论支撑。
selection_source: fresh_fetch
motivation: 旨在解决需要从相同初始状态重复执行的最优控制问题，并提高在线求解效率与策略改进的稳定性。
method: 提出一种基于轨迹驱动的在线策略迭代算法，利用当前策略生成的轨迹数据作为训练样本来迭代生成改进的后续策略。
result: 理论证明了在一致性条件下，该方法能确保给定初始状态下的策略成本单调改进，并在无人机避障和组合优化实验中验证了其有效性。
conclusion: 该框架融合了展开与策略迭代技术，为保证性能提升的神经网络策略训练提供了一种高效且具有理论支撑的途径。
---

## 摘要
我们考虑具有固定初始状态的确定性有限时域最优控制问题。我们引入了一种在线策略迭代方法，该方法从给定的初始策略（无论其如何获得）出发，生成一系列代价不断改进的策略及其相应的轨迹。每个策略产生一条轨迹，该轨迹反过来用于生成训练下一个策略的数据。该方法的动机源于从相同初始状态重复求解的问题，包括离散优化和重复性任务的路径规划。对于此类问题，该方法的速度足以实现在线应用。在自然的相容性条件下，我们证明了所生成策略的代价序列对于给定的初始状态是单调改进的（但不一定适用于其他状态）。我们通过组合优化和存在障碍物的无人机三维路径规划的计算研究来说明我们的结果。我们还简要讨论了该算法的随机对应版本。我们提出的框架结合了展期（rollout）和策略迭代的元素，以及灵活的基于轨迹的策略表示，并适用于涉及单个或多个决策者的问题。它还提供了一种利用轨迹数据训练基于神经网络的策略的有原则的方法，同时保持了代价的单调改进。
