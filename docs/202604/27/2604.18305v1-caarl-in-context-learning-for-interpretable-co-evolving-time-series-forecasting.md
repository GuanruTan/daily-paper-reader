...
tldr: 本研究针对具有复杂依赖和非平稳动态的协同演化时间序列预测问题，提出了一种名为CAARL的上下文感知自回归大语言模型框架。该方法将时间序列分解为自回归片段并构建时序依赖图，将其转化为叙述性文本，利用大语言模型的上下文学习能力生成类思维链的推理路径。CAARL在提高预测准确性的同时，通过显式的推理轨迹增强了模型的可解释性，为复杂时间序列分析提供了新思路。
selection_source: fresh_fetch
motivation: 旨在解决协同演化时间序列中复杂的依赖关系和非平稳动态带来的预测难题，并提升模型的可解释性。
method: 提出CAARL模型，通过将时间序列分解为自回归片段并构建时序依赖图，将其序列化为叙述性文本供大语言模型进行类思维链推理。
result: 在真实世界数据集上的实验验证了CAARL的有效性，证明其在保持高预测准确性的同时能提供透明的推理路径。
conclusion: CAARL作为一种兼具竞争力和高度可解释性的预测方法，展示了大语言模型在处理复杂动态时间序列数据中的巨大潜力。
---
