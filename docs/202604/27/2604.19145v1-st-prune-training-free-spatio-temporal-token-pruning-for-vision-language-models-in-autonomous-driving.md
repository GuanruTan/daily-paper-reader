...
tldr: "针对自动驾驶中视觉语言模型处理多视图多帧视频带来的巨大计算开销，本文提出ST-Prune框架。该框架无需训练且即插即用，包含运动感知时间剪枝（MTP）和环视空间剪枝（RSP）两个模块，通过利用时空冗余大幅减少Token数量。实验表明，在减少90% Token的情况下，该方法在感知、预测和规划任务中仍能保持近乎无损的性能，甚至在某些指标上超越基准模型。"
selection_source: fresh_fetch
motivation: 现有的Token剪枝方法多针对单图设计，无法有效处理自动驾驶场景中多视图和多帧输入带来的显著时空冗余。
method: 提出由MTP和RSP组成的框架，前者利用运动挥发性和时间近因性筛选动态轨迹，后者利用环视几何关系消除跨视图的重复投影。
result: "在四个基准测试中，ST-Prune在Token减少90%的情况下实现了近乎无损的性能，并达到了新的SOTA水平。"
conclusion: ST-Prune证明了通过合理的时空冗余消除，可以在不牺牲甚至提升性能的前提下，极大地提高自动驾驶VLMs的推理效率。
---

## 摘要
视觉语言模型（VLMs）已成为自动驾驶系统的核心，但其部署受到多视图摄像头和多帧视频输入带来的巨大计算开销的严重制约。现有的 Token 剪枝方法主要针对单图像输入设计，将每一帧或每一视图孤立对待，因此未能利用驾驶场景中固有的时空冗余。为了弥补这一差距，我们提出了 ST-Prune，这是一个免训练、即插即用的框架，由两个互补模块组成：运动感知时间剪枝（MTP）和环视空间剪枝（RSP）。MTP 通过将运动波动性和时间新近度编码为多样性选择目标中的软约束来解决时间冗余，优先考虑动态轨迹和当前帧内容，而非静态历史背景。RSP 通过利用环视摄像头几何结构来惩罚双边跨视图相似性，进一步解决空间冗余，消除仅靠时间剪枝无法抑制的重复投影和残留背景。这两个模块共同构成了一个完整的时空剪枝过程，在严格压缩下保留了关键场景信息。在涵盖感知、预测和规划的四个基准测试中进行的验证表明，ST-Prune 树立了免训练 Token 剪枝的新标杆。值得注意的是，即使在 Token 减少 90% 的情况下，ST-Prune 仍能实现近乎无损的性能，某些指标甚至超过了全模型基准，同时保持了与现有剪枝方法相当的推理速度。
