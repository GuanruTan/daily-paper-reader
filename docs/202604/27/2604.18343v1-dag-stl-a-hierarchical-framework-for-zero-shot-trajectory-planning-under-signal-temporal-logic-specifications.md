...
tldr: 针对未知动力学下的信号时序逻辑（STL）轨迹规划难题，本文提出DAG-STL分层框架。该框架将逻辑推理与轨迹生成解耦，通过分解STL公式、分配带时间戳的路标点以及利用扩散模型生成子轨迹，实现了在无任务先验数据下的零样本泛化。实验证明其在复杂长程任务中优于直接扩散方法，且具备高效的计算性能和良好的泛化能力。
selection_source: fresh_fetch
motivation: 现有的STL规划方法通常依赖显式动力学模型或特定任务学习，难以在未知动力学环境下实现对新任务的零样本泛化。
method: 提出一种“分解-分配-生成”的分层架构，将STL公式转化为带时间约束的路标点，并结合扩散模型和动态一致性指标进行轨迹合成与精炼。
result: 在Maze2D、AntMaze等多个导航与操纵任务中，DAG-STL在长程复杂任务上的表现显著优于基于鲁棒性引导的直接扩散方法。
conclusion: 该框架成功实现了逻辑推理与物理执行的有效结合，为未知环境下的复杂时序任务规划提供了一种高效且泛化性强的解决方案。
---
