...
## 摘要
降阶模型在分析和控制高维动力系统方面非常强大。然而，为足式机器人等复杂混合系统构建这些模型仍然具有挑战性。经典方法依赖于手工设计的模板模型（如 LIP、SLIP），虽然这些模型具有启发性，但仅能近似底层动力学。相比之下，数据驱动方法可以提取更准确的低维表示，但在隐空间中观察到的稳定性和安全性属性何时能有意义地转移回全阶系统仍不明确。为了弥补这一差距，我们引入了 HALO（混合自编码运动），这是一个直接从轨迹数据中学习周期性混合动力学隐降阶模型的框架。HALO 采用自编码器来识别低维隐状态，并结合学习到的隐庞加莱映射来捕捉步进运动动力学。这使得能够进行李雅普诺夫分析并在隐空间中构建相关的吸引域，两者都可以通过解码器映射回全阶状态空间。在模拟跳跃机器人和全身类人运动上的实验表明，HALO 产生的低维模型保留了有意义的稳定性结构，并能预测全阶吸引域边界。
