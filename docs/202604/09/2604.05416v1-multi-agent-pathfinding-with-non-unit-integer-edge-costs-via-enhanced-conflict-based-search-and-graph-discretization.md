...
tldr: 针对传统多智能体路径规划（MAPF）在处理非单位边权时的局限性，本文提出MAPFZ变体，通过非单位整数边权在保持有限状态空间的同时提升现实性。研究开发了CBS-NIC框架，结合时间间隔冲突检测与改进的SIPP算法，并引入贝叶斯优化图设计（BOGD）进行离散化。实验证明该方法在运行效率和成功率上均优于现有技术。
selection_source: fresh_fetch
motivation: 传统MAPF的单位边权假设限制了现实应用，而实值边权模型则因状态空间无限导致求解效率低下。
method: 提出了MAPFZ模型及CBS-NIC算法，结合时间间隔冲突检测、改进的SIPP和贝叶斯优化图设计方法。
result: 在多种基准测试中，该方法在运行时间和求解成功率方面均显著优于现有的最先进算法。
conclusion: 通过引入整数边权和优化的搜索框架，本研究在保证计算效率的同时，有效提升了非单位边权路径规划的实用性。
---

## 摘要
多智能体路径规划（MAPF）在多个领域发挥着关键作用。传统的 MAPF 方法通常假设单位边权和单时间步动作，这限制了它们在现实场景中的适用性。MAPFR 扩展了 MAPF 以处理具有实值边权和连续时间动作的非单位成本，但其几何碰撞模型导致了无限的状态空间，从而损害了求解器的效率。在本文中，我们提出了 MAPFZ，这是一种在具有非单位整数边权的图上的新型 MAPF 变体，它在保持有限状态空间的同时，提供了比经典 MAPF 更高的现实性。为了高效求解 MAPFZ，我们开发了 CBS-NIC，这是一个增强的基于冲突的搜索框架，结合了基于时间间隔的冲突检测和改进的安全间隔路径规划（SIPP）算法。此外，我们提出了用于图设计的贝叶斯优化（BOGD），这是一种针对非单位边权的离散化方法，在平衡效率和准确性的同时具有次线性遗憾界。大量实验表明，我们的方法在各种基准场景中的运行时间和成功率均优于现有最先进的方法。
