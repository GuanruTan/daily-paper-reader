...
## 摘要
由于能见度低、信号传输弱以及动态水流的影响，在未知环境中引导自主水下航行器（AUV）面临巨大挑战。这些因素给精确的全局定位、可靠的通信和避障带来了困难。局部感知提供了关键的实时环境数据，以支持在线决策。然而，水下传感器测量中固有的噪声引入了不确定性，使规划和控制变得复杂。为应对这些挑战，我们提出了一种集成规划与控制框架，该框架利用实时传感器数据动态引导闭环 AUV 轨迹，确保了鲁棒的避障能力并增强了在狭窄空间中的机动性。通过基于预先设计的反馈控制器进行运动规划，该方法降低了执行在线优化所需的计算复杂度，并提高了复杂水下空间中的作业安全性。所提方法通过在 RexRov AUV 上进行的 ROS Gazebo 仿真得到了验证，证明了其有效性。通过与基于 PID 的跟踪方法进行对比，并量化 AUV 进入目标通信范围时航位推算中的定位误差，对其性能进行了评估。
