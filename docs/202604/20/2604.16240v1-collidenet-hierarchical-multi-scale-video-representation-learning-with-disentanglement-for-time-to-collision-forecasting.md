...
## 摘要
碰撞时间 (TTC) 预测是碰撞预防中的一项关键任务，要求精确的时间预测，并理解视频中在空间和时间维度上封装的局部和全局模式。为了应对视频的多尺度特性，我们引入了一种名为 CollideNet 的新型时空层级化 Transformer 架构，专门用于高效的 TTC 预测。在空间流中，CollideNet 同时在多个分辨率下聚合每个视频帧的信息。在时间流中，除了多尺度特征编码外，CollideNet 还解耦了非平稳性、趋势和季节性成分。与之前的研究相比，我们的方法在三个常用的公开数据集上实现了最先进的性能，并以显著优势刷新了纪录。我们进行了跨数据集评估以分析该方法的泛化能力，并可视化了视频数据中趋势和季节性成分的解耦效果。我们在 https://github.com/DeSinister/CollideNet/ 发布了代码。
