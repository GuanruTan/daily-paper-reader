...
tldr: 针对自动驾驶在交互场景中缺乏长尾安全事件的问题，本文提出 Evaluation as Evolution ($E^2$) 闭环框架。该框架将对抗性场景生成从静态验证转变为自适应进化课程，通过在扩散模型先验上应用传输正则化稀疏控制和拓扑锚定技术，高效生成真实且具有挑战性的对抗场景。实验表明，$E^2$ 在 nuScenes 和 nuPlan 数据集上显著提升了碰撞故障发现率，并通过闭环微调增强了自动驾驶策略的鲁棒性。
selection_source: fresh_fetch
motivation: 静态数据集缺乏关键安全事件导致自动驾驶策略鲁棒性不足，且现有对抗测试多为开环，难以将发现的故障反馈至训练中。
method: 提出一种基于传输正则化稀疏控制和拓扑驱动支持选择的闭环框架，利用扩散模型先验生成受约束的对抗性场景。
result: "在 nuScenes 和 nuPlan 数据集上，碰撞故障发现率分别提升了 9.01% 和 21.43%，同时保持了高真实性。"
conclusion: 将对抗性评估转化为进化课程学习，能有效挖掘边界案例并显著提升自动驾驶策略的闭环鲁棒性。
---

## 摘要
交互式交通环境中的自动驾驶汽车通常受限于静态数据集中安全关键型长尾事件的稀缺，这使得学习到的策略偏向于平均情况行为并降低了鲁棒性。现有的评估方法试图通过对抗性压力测试来解决这一问题，但主要采用开环和事后分析的方式，难以将发现的故障案例重新整合到训练过程中。我们提出了“评估即进化”（$E^2$），这是一个闭环框架，将对抗生成从静态验证步骤转变为自适应进化课程。具体而言，$E^2$ 将对抗场景合成建模为对已学习的逆时间随机微分方程（SDE）先验的传输正则化稀疏控制。为了使这种高维生成变得可行，我们利用拓扑驱动的支撑集选择来识别关键的交互智能体，并引入拓扑锚定（Topological Anchoring）来稳定该过程。该方法能够有针对性地发现故障案例，同时严格限制与真实数据分布的偏差。实验表明，与最强的基准模型相比，$E^2$ 在 nuScenes 数据集上的碰撞故障发现率提高了 9.01%，在 nuPlan 数据集上提高了高达 21.43%，同时保持了较低的无效性和较高的真实性。此外，当将生成的边界案例循环用于闭环策略微调时，该方法显著提升了鲁棒性。
