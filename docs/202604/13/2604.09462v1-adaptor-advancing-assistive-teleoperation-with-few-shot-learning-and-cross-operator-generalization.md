...
tldr: 针对辅助远程操作中因操作员习惯和专业程度差异导致的意图识别不稳定问题，本文提出了 Adaptor 框架。该框架利用少样本学习，通过预处理阶段的噪声注入和关键帧提取来建模意图不确定性，并结合意图专家与视觉语言模型上下文，由动作专家生成指令。实验证明，Adaptor 在跨操作员泛化方面表现优异，显著提升了任务成功率和效率，实现了稳健的共享控制。
selection_source: fresh_fetch
motivation: 解决辅助远程操作中由于不同操作员习惯和专业水平差异导致的轨迹分布异质性及意图识别不稳定问题。
method: 提出一种两阶段框架，通过合成轨迹扰动建模意图不确定性，并结合意图专家与预训练视觉语言模型来指导动作生成。
result: 在真实世界和模拟基准测试中达到了 SOTA 性能，显著提高了任务成功率，并降低了不同操作员之间的表现差异。
conclusion: Adaptor 实现了鲁棒的跨操作员泛化，为高效、稳定的辅助远程操作提供了有效的少样本学习方案。
---
