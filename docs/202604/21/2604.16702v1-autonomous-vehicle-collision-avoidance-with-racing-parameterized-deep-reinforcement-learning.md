...
tldr: 本研究针对人类驾驶失误导致的交通事故，提出一种基于赛车超车参数化的深度强化学习（DRL）避障策略。该方法利用物理感知和模拟器漏洞感知的奖励函数，在不依赖显式几何参考轨迹的情况下，实现了车辆动力学极限下的紧急避障。实验表明，该策略在计算效率和避障成功率上均显著优于传统的MPC-APF基准，并成功实现了硬件的零样本迁移。
selection_source: fresh_fetch
motivation: 旨在解决由人类驾驶失误引起的高致死率交通事故，提升自动驾驶车辆在极限动力学下的紧急避障能力。
method: 采用赛车超车场景参数化的深度强化学习，结合物理感知奖励函数，在无参考轨迹引导下训练避障策略。
result: 该策略在推理延迟上降低了64倍，且在对头碰撞和侧面碰撞场景中的避障表现均优于MPC-APF基准及数值最优控制。
conclusion: 赛车参数化的DRL策略为自动驾驶提供了一种高效率、高性能的紧急避障方案，具有极强的实战应用潜力。
---

## 摘要
道路交通事故是全球范围内导致死亡的主要原因之一。在美国，人为错误导致了 94% 的车祸，每年造成超过 7,000 名行人死亡和 5,000 亿美元的经济损失。配备紧急避障系统的自动驾驶车辆（AVs）在高频下运行于车辆动力学极限，面临非线性运动动力学精度和计算效率的双重约束，这在恶劣天气和网络安全漏洞期间进一步增强了安全性，并在自动驾驶车辆与人类驾驶员共享道路时能够规避危险的人为驾驶。本文利用赛车超车场景，在仿真中参数化了一种分布外（OOD）深度强化学习（DRL）避障策略，无需显式的几何模拟参考轨迹引导，并采用物理信息驱动且感知仿真器漏洞的奖励函数来编码非线性车辆运动动力学。本文评估了两种策略：默认的单向策略和反向航向变体（即与其它车辆反向行驶），两者在三种交叉路口碰撞场景中均一致优于模型预测控制与人工势场（MPC-APF）基准，并实现了向等比例缩放硬件的零样本迁移，且浮点运算量（FLOPS）减少了 31 倍，推理延迟降低了 64 倍。在正面碰撞中，反向航向策略的表现比默认赛车超车策略高出 30%，比基准高出 50%；在侧面碰撞中，其表现与前者相当，且两种 DRL 策略的规避率均比数值最优控制高出 10%。
