...
tldr: 本文针对多源异构动态系统建模中难以提取共享模式的问题，提出了一种分布鲁棒学习方法。该方法通过在轨迹导数的凸组合构成的不确定性集合中最大化最坏情况奖励，构建鲁棒动态系统。研究引入了双层稳定化程序以增强估计稳定性，并提供了严谨的理论保证。实验证明，该方法在模拟和颅内脑电图数据分析中均显著提升了泛化性能。
selection_source: fresh_fetch
motivation: 现有的常微分方程建模方法多针对单一系统，难以有效处理多个异构动态系统间的共享模式学习问题。
method: 提出一种分布鲁棒学习框架，通过二次优化确定权重以平衡多源信息，并结合双层稳定化程序提高估计的鲁棒性。
result: 理论上证明了权重的相合性、轨迹估计的误差界及置信区间的渐近有效性，并在实际脑电数据中展现了优越的泛化能力。
conclusion: 该方法为异构动态系统的鲁棒建模提供了有效的理论框架和计算工具，显著提升了跨系统学习的性能。
---

## 摘要
常微分方程（ODE）为建模广泛科学领域中出现的动力系统提供了一个强大的框架。然而，大多数现有的 ODE 方法都集中在单一系统上，未能充分解决从多个异构动力系统中学习共享模式的问题。在本文中，我们提出了一种用于建模异构 ODE 系统的创新分布鲁棒学习方法。具体而言，我们通过在由轨迹导数的凸组合构成的不确定性集合上最大化最坏情况下的奖励，构建了一个鲁棒动力系统。我们证明了所得估计量具有显式的加权平均表示，其中权重通过平衡多个数据源信息的二次优化获得。我们进一步开发了一种双层稳定化程序，以解决估计中潜在的不稳定性。我们为所提方法建立了严格的理论保证，包括稳定权重的相合性、鲁棒轨迹估计的误差界以及逐点置信区间的渐近有效性。通过广泛的模拟和对颅内脑电图数据的分析，我们证明了与替代方案相比，所提方法显著提高了泛化性能。
