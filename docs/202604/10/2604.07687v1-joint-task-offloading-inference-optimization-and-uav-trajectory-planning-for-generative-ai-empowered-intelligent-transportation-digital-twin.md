...
title_zh: 生成式人工智能赋能的智能交通数字孪生中联合任务卸载、推理优化与无人机轨迹规划
...
## 摘要
为了实现智能交通数字孪生（ITDT），调度无人机（UAV）来处理来自路侧传感器的感知数据。此时，在无人机上部署扩散模型等生成式人工智能（GAI）技术，将原始感知数据转化为高质量且有价值的数据。因此，我们提出了生成式人工智能赋能的 ITDT。在具有动态移动性的无人机上动态处理一组扩散模型推理（DMI）任务，会同时影响数字孪生（DT）更新的保真度和延迟。本文研究了 DMI 任务卸载、推理优化和无人机轨迹规划的联合优化问题，将其建模为系统效用最大化（SUM）问题，以解决生成式人工智能赋能的 ITDT 中的保真度与延迟权衡。为了在网络动态环境下寻求该问题的解，我们将 SUM 问题建模为异构多智能体马尔可夫决策过程，并提出了基于顺序更新的异构多智能体双延迟深度确定性策略梯度（SU-HATD3）算法，该算法能够快速学习到近优解。数值结果表明，与几种基准算法相比，所提算法在提高系统效用和收敛速度方面具有显著优势。
