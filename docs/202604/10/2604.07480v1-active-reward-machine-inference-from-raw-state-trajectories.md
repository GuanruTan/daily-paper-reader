...
tldr: 针对多阶段任务中奖励机手动设计困难的问题，本文提出了一种直接从原始状态轨迹和策略信息中推断奖励机的方法。该研究在不依赖奖励信号、标签或内部节点信息的极端数据稀缺环境下，确定了学习所需的充分轨迹数据条件，并引入主动学习机制通过增量查询轨迹扩展来提升学习效率，在网格世界实验中验证了其有效性。
selection_source: fresh_fetch
motivation: 手动指定奖励机及其高层特征标注函数非常困难，且现有方法通常依赖于难以获取的奖励或标签信息。
method: 提出一种从原始状态和策略数据中推断奖励机的算法，并结合主动学习策略通过增量查询轨迹扩展来优化数据效率。
result: 确定了在缺乏奖励和标签信息时学习奖励机所需的充分轨迹数据条件，并在多个网格世界示例中成功验证。
conclusion: 该方法证明了在极低信息环境下自动推断复杂任务逻辑结构的可行性，显著降低了机器人策略合成中对人工先验知识的依赖。
---
