...
tldr: 针对自主机器人在复杂环境中面临的安全性挑战，本文提出SCRAMPPI框架。该方法将应急规划建模为带有硬性安全约束的轨迹优化问题，利用Hamilton-Jacobi (HJ) 可达性分析在线计算安全集的后向可达集价值函数。通过将该函数与采样规划器MPPI结合，在保证从标称路径任意点均能安全撤退的同时，显著提升了采样效率。实验证明该算法能在对抗性规避任务中实时生成可靠的标称与应急计划。
selection_source: fresh_fetch
motivation: 传统的机器人规划方法在应对突发状况时，往往难以在保证硬性安全约束的同时兼顾计算效率。
method: 结合HJ可达性分析与MPPI采样规划器，通过在线计算安全价值函数来指导重采样，确保应急路径的可行性。
result: 在仿真和硬件实验中，该算法成功实现了在对抗性环境下实时生成满足安全约束的导航轨迹。
conclusion: SCRAMPPI通过融合形式化验证与随机优化，为移动机器人提供了一种高效且具有安全保障的应急规划方案。
---

## 摘要
自主机器人通常旨在完成名义行为并最小化代价；这使得它们在面对故障或计划外场景时显得脆弱，此时需要一个通往安全集的备份或应急计划，以避免任务彻底失败。这被形式化为一个带有安全约束的名义代价轨迹优化问题：在名义计划的任何一点上，必须存在一条通往指定安全集的可行轨迹。以往的方法要么放宽这一硬约束，要么使用昂贵的基于采样的策略来优化该约束。相反，我们将这一要求形式化为“到达-规避”（reach-avoid）问题，并利用 Hamilton-Jacobi (HJ) 可达性分析来验证应急可行性。通过在环境显现时在线计算安全集后向可达集的价值函数，并利用基于重采样的展开（rollouts）将其与基于采样的规划器 (MPPI) 集成，我们在保证满足硬约束的同时，大幅提高了采样效率。最后，我们展示了仿真和硬件实验，证明了我们的算法在对抗性规避任务中，能够在移动机器人上实时生成名义计划和应急计划。
