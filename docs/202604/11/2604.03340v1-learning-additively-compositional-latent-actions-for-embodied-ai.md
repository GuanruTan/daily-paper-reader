...
tldr: 针对具身智能中从视频学习潜在动作时缺乏物理运动结构先验的问题，本文提出了AC-LAM模型。该模型通过在潜在动作空间引入加性组合结构约束（如恒等、逆向和循环一致性），使学习到的动作表示具备代数结构，有效减少了场景细节干扰并校准了运动幅度。实验证明，AC-LAM在模拟和真实环境任务中均优于现有方法，为下游策略学习提供了更强的监督信号。
selection_source: fresh_fetch
motivation: 现有的潜在动作学习方法缺乏对物理运动加性组合结构的建模，导致动作表示容易与场景细节纠缠且运动幅度不准确。
method: 提出了AC-LAM模型，通过在短时域内施加恒等、逆变换和循环一致性等代数约束，强制潜在动作空间具备加性组合特性。
result: 实验表明AC-LAM学习到了更具结构化和位移校准的潜在动作，在模拟及真实世界的桌面操作任务中均取得了领先的性能。
conclusion: 引入物理运动的代数结构先验能显著提升潜在动作学习的质量，从而为具身智能策略提供更有效的监督和更好的泛化能力。
---

## 摘要
潜动作学习通过从视觉转换中推断伪动作标签，为具身智能利用互联网规模的视频数据提供了一种途径。然而，大多数方法在学习潜动作时缺乏能够编码物理运动加性组合结构的结构先验。因此，潜变量往往会将无关的场景细节或未来观测信息与真实的状态变化相纠缠，并导致运动幅度的校准失准。我们提出了加性组合潜动作模型（AC-LAM），该模型在短时域内的潜动作空间上强制执行场景级的加性组合结构。这些加性组合约束促进了潜动作空间中简单代数结构（如恒等、逆、循环一致性）的形成，并抑制了不符合加性组合性质的信息。实验结果表明，AC-LAM 学习到了更具结构化、运动特定且位移校准更准确的潜动作，为下游策略学习提供了更强的监督信号，在模拟和真实世界的桌面任务中表现均优于现有的先进潜动作模型。
