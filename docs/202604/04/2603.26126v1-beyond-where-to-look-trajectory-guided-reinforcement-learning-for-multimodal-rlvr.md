...
## 摘要
多模态大语言模型（MLLM）中具有可验证奖励的强化学习（RLVR）的最新进展主要集中在提高最终答案的正确性和加强视觉定位上。然而，一个关键瓶颈仍然存在：尽管模型能够关注相关的视觉区域，但它们往往无法有效地将视觉证据整合到后续的推理中，导致推理链与视觉事实的关联性较弱。为了解决这一问题，我们提出了轨迹引导强化学习（TGRL），该方法利用来自更强模型的专家推理轨迹，引导策略模型将视觉证据整合到细粒度的推理过程中。我们进一步引入了词元级重加权和轨迹过滤，以确保稳定且有效的策略优化。在多个多模态推理基准测试上的广泛实验表明，TGRL 持续提升了推理性能，并有效弥合了视觉感知与逻辑推理之间的差距。
