...
tldr: 针对自动驾驶事故预测模型在面对微小扰动时表现出的预测和潜在表示不稳定性问题，本文提出了SECURE框架。该框架通过定义预测空间和潜在特征空间的连贯性与稳定性，引入了一种多目标损失函数的训练方法，旨在减少对对抗扰动的敏感性。实验证明，SECURE不仅显著增强了模型在多种扰动下的鲁棒性，还提升了在干净数据上的表现，达到了新的SOTA水平。
selection_source: fresh_fetch
motivation: 现有的事故预测模型在面对现实世界的微小输入扰动时，其预测结果和潜在表示存在严重的不稳定性，威胁自动驾驶安全。
method: 提出了SECURE框架，通过多目标损失函数进行微调，在预测空间和潜在特征空间同时强制执行一致性和稳定性约束。
result: 在DAD和CCD数据集上的实验表明，该方法在显著提高鲁棒性的同时，也刷新了干净数据上的性能记录。
