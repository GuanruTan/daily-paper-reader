...
tldr: 本文针对离线、零样本信号时序逻辑（STL）规划问题，提出了GraSP-STL框架。在无动力学模型和环境交互的约束下，该方法利用离线任务无关数据学习目标条件价值函数，并据此构建状态空间的有向图抽象。通过在图上搜索满足STL规范的路径点序列，并结合学习到的目标条件策略执行，实现了对未见STL任务的零样本泛化。该框架将可重用的可达性学习与任务规划分离，有效解决了长时程复杂逻辑任务的规划难题。
selection_source: fresh_fetch
motivation: 旨在解决仅依赖离线任务无关数据且无环境交互的情况下，如何实现对任意未知STL规范的零样本控制策略合成。
method: 结合离线目标条件强化学习与图搜索，利用价值函数诱导的可达性度量构建图抽象，并通过算术几何平均鲁棒性评估路径点序列。
result: 在多项离线STL规划任务中验证了其有效性，展示了强大的零样本泛化能力和处理长时程任务的能力。
conclusion: GraSP-STL通过解耦可达性学习与逻辑规划，为基于离线数据的复杂时序逻辑任务规划提供了一种高效且通用的解决方案。
---

## 摘要
本文研究了信号时序逻辑（STL）规范下的离线零样本规划。我们假设仅能访问由任务无关的行为策略收集的状态-动作-状态转移离线数据集，且不具备解析动力学模型，无需进一步的环境交互，也不进行特定任务的重新训练。目标是合成一种控制策略，使其生成的轨迹满足任意未见的 STL 规范。为此，我们提出了 GraSP-STL，这是一个基于图搜索的框架，用于从离线轨迹中进行零样本 STL 规划。该方法从离线数据中学习目标条件价值函数，并利用其在状态空间上推导出有限时域可达性度量。基于该度量，它构建了一个有向图抽象，其节点表示代表性状态，边编码了可行的短时域转移。规划随后被表述为对路标点序列的图搜索，使用算术-几何平均鲁棒性及其区间语义进行评估，并由学习到的目标条件策略执行。所提出的框架将可重用的可达性学习与任务条件规划分离，从而实现了对未见 STL 任务的零样本泛化，并通过组合离线数据中的短时域行为实现长时域规划。实验结果证明了该方法在多种离线 STL 规划任务中的有效性。
