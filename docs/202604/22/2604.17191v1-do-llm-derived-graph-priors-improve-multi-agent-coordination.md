...
tldr: 本研究探讨了利用大语言模型（LLM）为多智能体强化学习（MARL）生成协调图先验的可能性。针对现有方法在确定智能体协调结构时存在的脆弱性或高数据需求，作者提出通过自然语言描述智能体观测，利用LLM推断潜在协调模式，并将其集成到基于图神经网络的MARL框架中。在MPE基准测试上的实验表明，该方法能显著提升协调效率与适应性，且仅需1.5B参数的小模型即可生成高质量先验。
selection_source: fresh_fetch
motivation: 现有的多智能体协调图构建方法通常依赖手动设定或从零学习，缺乏语义信息且数据效率低下。
method: 利用LLM根据智能体观测的自然语言描述生成协调图先验，并将其通过图卷积层集成到GNN-MARL算法中。
result: 在多个MPE场景中，LLM生成的先验显著提升了智能体的协调性能，且1.5B规模的小模型表现优异。
conclusion: LLM衍生的图先验能有效增强动态多智能体环境中的协调与适应能力，为结合语义理解与强化学习提供了新路径。
---
