...
motivation: 现有预测系统将预测、物理约束和可靠性验证分离，限制了系统的可扩展性、可解释性及在关键科学领域的应用。
method: 引入双曲流形几何AI框架，利用负曲率诱导的收缩动力学实现内在鲁棒性，并通过分层架构整合物理定律与形式化验证。
result: "该方法在多个领域达到最先进的准确率，同时将计算成本降低了97.5%，并提供了更优的认证率。"
conclusion: GeoCert通过将验证机制嵌入学习几何中，实现了从经验近似到形式化验证推理的转变，为构建可信科学AI奠定了基础。
---

## 摘要
科学领域的预测系统必须准确、符合物理规律且具有可验证的可靠性。现有的大多数模型将预测、约束执行和验证分开处理，限制了其可扩展性和可解释性。我们提出了 GeoCert，这是一个几何人工智能框架，它在单一的可微计算中统一了预测、物理推理和形式化验证。GeoCert 将预测公式化为沿双曲流形的演化，其中负曲率诱导了收缩动力学、内在鲁棒性和对数时间验证。分层约束架构将通用物理定律与特定领域的动力学分离，实现了在能源、气候、金融和交通系统中的可验证泛化。GeoCert 在达到最先进准确率的同时，将计算成本降低了 97.5%，并保持了更高的验证率。通过将验证嵌入到学习的几何结构中，GeoCert 将预测从经验近似转变为形式化验证的推理，为构建可靠、可复现且具有物理基础的科学人工智能提供了可扩展的基础。
