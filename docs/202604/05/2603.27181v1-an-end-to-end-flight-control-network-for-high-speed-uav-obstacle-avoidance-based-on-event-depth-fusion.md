...
score: 6.0
evidence: 无人机自主飞行与避障
tldr: "针对无人机在复杂动态环境下的高速避障难题，本文提出一种基于深度图与事件相机数据融合的端到端飞行控制网络。通过双向交叉注意力机制实现特征级融合，弥补了单一传感器在高速运动或静态场景下的局限。结合高效的球面主搜索（SPS）专家规划器进行模仿学习，该方法在17m/s的高速飞行中实现了70-80%的避障成功率，显著优于单模态及单向融合方案。"
selection_source: fresh_fetch
motivation: 单一传感器在高速飞行或处理动静混合障碍物时存在感知局限，难以同时兼顾静态场景识别与动态运动捕捉。
method: 采用双向交叉注意力模块进行深度与事件数据的特征级融合，并利用新设计的球面主搜索（SPS）算法作为专家规划器进行模仿学习训练。
result: "在17m/s的高速测试中，该方法实现了70-80%的避障成功率，性能比单模态或单向融合模型提升了10-20%。"
conclusion: 双向融合机制能有效整合互补的传感器信息，为无人机在复杂环境下的高速自主飞行提供了更可靠的控制方案。
---

## 摘要
在包含静态、动态或混合障碍物的复杂环境中实现安全、高速的自主飞行仍具挑战性，因为单一感知模态往往是不完整的。深度相机对静态物体有效，但在高速下会受到运动模糊的影响。相反，事件相机擅长捕捉快速运动，但在感知静态场景时较为困难。为了利用这两种传感器的互补优势，我们提出了一种端到端飞行控制网络，通过双向交叉注意力模块实现深度图像与事件数据的特征级融合。该端到端网络通过模仿学习进行训练，而模仿学习依赖于高质量的监督信号。基于此，我们利用球面主搜索（SPS）设计了一种高效的专家规划器。该规划器将计算复杂度从 $O(n^2)$ 降低至 $O(n)$，同时能生成更平滑的轨迹，在 17m/s 的速度下实现了超过 80% 的成功率，比传统规划器高出近 20%。仿真实验表明，我们的方法在各种场景下、17 m/s 的速度下均达到了 70-80% 的成功率，优于单模态和单向融合模型 10-20%。这些结果证明，双向融合能有效整合事件与深度信息，从而在包含静态和动态物体的复杂环境中实现更可靠的避障。
