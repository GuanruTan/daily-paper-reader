...
tldr: 针对工业安全和机器人感知的全景3D多目标跟踪需求，本文提出S3KF框架。该框架利用旋转式LiDAR和四鱼眼相机阵列，通过单位球面上的几何一致性状态表示，解决了全景图像畸变和传统欧式空间参数冗余问题。通过扩展球面卡尔曼滤波融合多模态数据，实现了高精度的深度与尺度估计。实验证明该系统在实时运行下达到了分米级跟踪精度，显著提升了动态场景下的身份连续性。
selection_source: fresh_fetch
motivation: 传统图像平面跟踪器受全景畸变影响大，而欧式3D建模存在参数冗余且难以统一处理角度、尺度与深度。
method: 提出一种基于单位球面的状态表示法，利用切平面参数化建模目标方位，并结合扩展卡尔曼滤波融合全景相机与LiDAR观测。
result: 在真实场景实验中实现了分米级平面跟踪精度，相比2D全景基准提升了身份连续性，并在嵌入式平台上达到实时性能。
conclusion: S3KF为工业级全景感知提供了一种实用且高效的3D多目标跟踪解决方案，有效平衡了计算效率与跟踪鲁棒性。
---

## 摘要
全景多目标跟踪对于工业安全监控、广域机器人感知以及大型工作空间中的轻量化基础设施部署具有重要意义。在这些场景下，传感系统必须在宽视野畸变和遮挡的情况下，提供全方位覆盖、度量几何线索以及稳定的目标关联。现有的图像平面跟踪器与相机投影紧密耦合，在全景图像中变得不可靠，而传统的欧几里得 3D 公式引入了冗余的方向参数，且无法自然地统一角度、尺度和深度估计。在本文中，我们提出了 S3KF，这是一个基于电动旋转激光雷达和四鱼眼相机装置的全景 3D 多目标跟踪框架。其核心思想是单位球面 $\mathbb{S}^2$ 上几何一致的状态表示，其中目标方位通过两自由度切平面参数化进行建模，并与边界框尺度和深度动力学进行联合估计。基于该状态，我们推导了一种扩展球面卡尔曼滤波流程，融合全景相机检测与激光雷达深度观测，实现多模态跟踪。我们还建立了一个基于地图的真值生成流程，利用注册到共享全局激光雷达地图的穿戴式定位设备，在无需动作捕捉基础设施的情况下实现定量评估。在自采集的真实世界序列上的实验表明，该方法实现了分米级平面跟踪精度，在动态场景中相比 2D 全景基准提升了身份连续性，并在 Jetson AGX Orin 平台上实现了实时机载运行。这些结果表明，所提出的框架是全景感知和工业级多目标跟踪的实用解决方案。项目页面见 https://kafeiyin00.github.io/S3KF/。
