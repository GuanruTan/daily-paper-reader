...
conclusion: Step-MPPI通过将长时域目标嵌入神经分布策略，成功平衡了控制性能与实时计算效率。
---

## 摘要
模型预测路径积分 (MPPI) 是一种用于解决复杂模型预测控制 (MPC) 问题的基于采样的方法，但其实时实现面临两个关键挑战：计算成本和样本需求随预测时域增加而增长，且手动调节采样协方差需要平衡探索与噪声。为解决这些问题，我们提出了 Step-MPPI，这是一个学习采样分布以实现高效单步前瞻 MPPI 的框架。具体而言，我们使用神经网络对每个时间步的 MPPI 提议分布进行参数化，并利用 MPC 代价、约束惩罚和最大熵正则化项，在长时域内以自监督方式对其进行训练。通过将长时域目标嵌入到神经分布策略的训练中，Step-MPPI 实现了多步优化器的远见，同时具备单步前瞻的毫秒级延迟。我们在多个具有挑战性的任务中证明了 Step-MPPI 的效率，在这些任务中，MPPI 往往受限于高维数和/或长控制时域。
