...
tldr: 针对未来无线通信中多任务预测需求及监督学习适配成本高的问题，本文提出JEPA-MSAC框架。该框架采用自监督学习，将多模态感知与通信数据映射至统一空间，通过时间块掩码的联合嵌入预测架构（JEPA）预训练共享骨干网络。该网络能捕捉环境动态和跨模态依赖，仅需冻结骨干并添加轻量化任务头，即可高效实现定位、波束预测及RSSI预测等多项任务。
selection_source: fresh_fetch
motivation: 现有的无线通信学习方法多针对单一任务设计，在动态环境下缺乏通用的预测性表征且适配成本高。
method: 提出一种自监督多模态预测表征学习框架，利用时间块掩码JEPA预训练共享骨干网络以捕获环境动态和跨模态关联。
result: 实验证明该框架生成的潜在特征能以极低的适配成本支持高精度的定位、波束预测和RSSI预测等多任务。
conclusion: JEPA-MSAC验证了自监督预训练在无线通信多模态表征学习中的有效性，为构建通用物理层预测模型提供了新方案。
---

## 摘要
未来无线系统日益需要能够支持动态环境下多种物理层（PHY）任务的预测性且可迁移的表征。然而，大多数现有的基于监督学习的方法是针对单一任务设计的，这导致了较高的适配成本。为了解决这一问题，我们提出了一种用于多模态感知辅助通信的联合嵌入预测架构（JEPA-MSAC），这是一种针对无线环境的自监督多模态预测表征学习框架。该框架首先将多模态感知和通信测量数据映射到统一的 token 空间，然后使用时间块掩码 JEPA 预训练共享骨干网络，以学习一个能够捕捉环境动态和跨模态依赖关系的预测性潜空间。预训练完成后，骨干网络被冻结并作为通用的未来特征生成器重复使用，在其基础上训练轻量级任务头，用于定位、波束预测和接收信号强度指示（RSSI）预测。广泛的实验表明，该潜状态能够以较低的适配成本支持准确的多任务预测。此外，消融研究揭示了其缩放行为以及关键预训练设置的影响。
