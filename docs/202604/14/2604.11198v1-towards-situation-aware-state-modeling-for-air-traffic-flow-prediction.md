...
tldr: 针对终端区空中交通流量预测中时间序列模型忽略微观状态信息的问题，本文提出AeroSense框架。该框架摒弃了先聚合再建模的传统范式，直接将实时空域态势建模为动态飞机状态集。通过情境感知状态表示、掩码自注意力机制及解耦预测头，AeroSense能直接从微观运动学数据中捕捉交互与异构动态。实验证明其在预测精度、高峰期鲁棒性及可解释性上均优于现有SOTA方法。
selection_source: fresh_fetch
motivation: 传统基于时间序列的流量预测方法因预先聚合轨迹而丢失了飞机实时运动学及空域边界等关键微观状态信息。
method: 提出AeroSense框架，利用情境感知状态表示和掩码自注意力机制，直接从动态飞机状态集中建模空域交互与流量动态。
result: 在大规模真实机场数据集上的实验显示，该方法在预测精度上达到SOTA，且在交通高峰期表现出更强的鲁棒性。
conclusion: 直接对微观飞机状态进行建模比传统时间序列方法具有更高的预测保真度，为主动式空中交通管理提供了更有效的技术手段。
---
