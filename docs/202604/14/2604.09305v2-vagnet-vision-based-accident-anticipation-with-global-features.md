...
date: 2026-04-10
pdf: "https://arxiv.org/pdf/2604.09305v2"
tags: ["query:tp"]
score: 7.0
evidence: 自动驾驶和交通场景中的事故预判
tldr: 针对交通事故预防，本文提出VAGNet模型。传统方法依赖计算密集的物体级特征，而VAGNet利用VideoMAE-V2提取全局特征，并结合Transformer和图模块进行事故预测。在DAD、DoTA等四个数据集上的实验证明，该方法在提高预测精度和提前预警时间的同时，显著提升了计算效率，为自动驾驶安全提供了实时高效的解决方案。
selection_source: fresh_fetch
motivation: 现有的事故预测方法通常需要提取每个检测对象的特征，计算开销巨大，难以满足实时性需求。
method: 提出一种基于全局特征的深度神经网络VAGNet，利用VideoMAE-V2提取特征，并结合Transformer和图模块进行建模，无需显式的物体级特征。
result: 在DAD、DoTA、DADA和Nexar四个基准数据集上，VAGNet在平均精度（AP）和平均事故提前时间（mTTA）上均优于现有方法。
conclusion: VAGNet证明了仅使用全局特征即可实现高效且准确的事故预测，为实时驾驶辅助系统提供了一种更具计算优势的方案。
---

## 摘要
交通事故是全球范围内导致伤亡的主要原因。因此，提前预判危险情况的能力至关重要。自动事故预判能够通过驾驶员警报和避障操作实现及时干预，是高级驾驶辅助系统的关键组成部分。在自动驾驶中，这种预测能力支持主动安全行为，例如在需要时启动防御性驾驶和人工接管。使用行车记录仪视频作为输入提供了一种极具成本效益的解决方案，但由于现实驾驶场景的复杂性，这仍具挑战性。事故预判系统需要实时运行，然而，当前的方法通常涉及从每个检测到的目标中提取特征，这会产生巨大的计算开销。我们提出了 VAGNet，这是一种深度神经网络，它利用交通场景的全局特征从行车记录仪视频中学习预测事故，而无需显式的目标级特征。该网络由 Transformer 和图模块组成，并采用视觉基础模型 VideoMAE-V2 进行全局特征提取。在四个基准数据集（DAD、DoTA、DADA 和 Nexar）上的实验表明，与现有方法相比，我们的方法在预判事故时具有更高的平均精度和平均事故提前时间，同时计算效率更高。
