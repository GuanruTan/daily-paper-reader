...
tldr: 本研究提出Rays as Pixels，一种学习视频与相机轨迹联合分布的视频扩散模型（VDM）。该模型将相机参数表示为稠密射线像素（raxels），通过解耦自交叉注意力机制实现视频帧与轨迹的联合去噪。它能同时处理相机轨迹预测、视频与轨迹联合生成以及受控视频生成三项任务。实验表明，该模型在位姿估计和相机控制视频生成方面表现出色，且通过闭环自一致性测试验证了其预测的准确性与高效性。
selection_source: fresh_fetch
motivation: 传统的相机参数恢复与新视角合成任务相互分离，在图像稀疏或位姿模糊时难以协同工作。
method: 将相机表示为稠密射线像素，并利用视频扩散模型中的解耦自交叉注意力机制对视频和轨迹进行联合建模。
result: 模型在位姿估计和相机受控视频生成任务上取得了显著成果，且轨迹预测所需的去噪步数远少于视频生成。
conclusion: 通过学习视频与相机轨迹的联合分布，该模型实现了相机感知与内容生成的统一，提升了复杂场景下的鲁棒性。
---
