...
conclusion: 利用交通参与者的轨迹信息能有效弥补视觉感知的不足，显著增强复杂驾驶场景下高精地图构建的稳定性和准确性。
---

## 摘要
高精地图（HD mapping）任务涉及车道线检测与预测，由于视线遮挡、远距离车道可见度低以及恶劣天气等非理想条件，具有极大的挑战性。这些条件通常会导致车道检测精度受损，并降低自动驾驶系统的可靠性。为了应对这些挑战，我们提出了 MapATM，这是一种新型深度神经网络，它有效地利用历史参与者轨迹信息（此处参与者指行驶中的车辆）来提高车道检测精度。通过将参与者轨迹作为道路几何结构的结构先验，MapATM 实现了显著的性能提升，在具有挑战性的 NuScenes 数据集上，车道分隔线的 AP 提升了 4.6，mAP 提升了 2.6，与强基线方法相比，分别实现了 10.1% 和 6.1% 的相对提升。广泛的定性评估进一步证明了 MapATM 在各种复杂驾驶场景下持续保持稳定且鲁棒的地图重建能力，凸显了其在自动驾驶应用中的实际价值。
