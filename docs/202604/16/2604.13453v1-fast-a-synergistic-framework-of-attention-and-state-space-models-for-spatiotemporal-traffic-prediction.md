...
tldr: 针对交通预测中Transformer计算复杂度高和Mamba空间建模能力不足的问题，本文提出FAST框架。该框架采用“时间-空间-时间”架构，结合注意力机制捕捉时序模式，并利用Mamba模块以线性复杂度建模长程空间依赖。通过引入多源时空嵌入和多级跳跃预测机制，FAST在多个基准数据集上显著提升了预测精度，实现了性能与效率的平衡。
selection_source: fresh_fetch
motivation: 现有的交通预测方法在捕捉全局空间依赖的表达能力与计算效率之间存在难以兼顾的权衡。
method: 提出一种结合时间注意力与Mamba空间模块的协同框架，并引入多源时空嵌入和多级特征融合机制。
result: 在PeMS04/07/08数据集上，FAST在MAE和RMSE指标上均优于Transformer、GNN及Mamba类基准模型。
conclusion: FAST成功融合了注意力机制与状态空间模型的优势，为大规模时空交通预测提供了一种高效且准确的解决方案。
---
