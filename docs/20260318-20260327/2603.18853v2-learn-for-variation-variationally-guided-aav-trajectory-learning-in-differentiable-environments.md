...
tags: ["query:tp"]
score: 8.0
evidence: 无人机的梯度告知轨迹学习
tldr: 针对6G物联网中无人机（AAV）轨迹规划面临的奖励稀疏和训练不稳定问题，本文提出了一种名为L4V的梯度引导学习框架。该框架通过构建端到端可微的计算图，将AAV运动学、信道增益和数据采集过程整合，利用随时间反向传播（BPTT）获取精确的策略梯度，从而替代传统强化学习中的标量奖励信号，显著提升了轨迹规划的效率和稳定性。
selection_source: fresh_fetch
motivation: 传统强化学习在无人机轨迹规划中存在严重的信用分配问题和训练不稳定性，难以捕捉序列运动的长期非线性影响。
method: 提出L4V框架，通过构建可微环境并利用随时间反向传播（BPTT）直接计算目标函数对控制动作的精确梯度来训练确定性神经策略。
result: 仿真结果表明，L4V在任务完成时间、平均传输速率和训练成本方面均显著优于遗传算法、DQN、A2C和DDPG等主流基准方法。
conclusion: 将环境动力学转化为可微计算图并利用结构化梯度引导学习，是解决复杂移动物联网场景下无人机轨迹优化问题的有效途径。
---
