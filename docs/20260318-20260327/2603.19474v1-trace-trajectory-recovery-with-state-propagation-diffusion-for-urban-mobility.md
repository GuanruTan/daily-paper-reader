...
title_zh: TRACE：基于状态传播扩散模型的城市移动轨迹恢复
authors: "Jinming Wang, Hai Wang, Hongkai Wen, Geyong Min, Man Luo"
date: 2026-03-19
pdf: "https://arxiv.org/pdf/2603.19474v1"
tags: ["query:tp"]
score: 8.0
evidence: 轨迹恢复与时空模式重建
tldr: 针对城市移动轨迹数据稀疏且分布不均的问题，本文提出了一种名为TRACE的新型扩散模型。该模型核心是状态传播扩散模型（SPDM），通过引入创新的记忆机制，在去噪过程中保留并利用前一步的中间结果，从而有效恢复复杂的轨迹片段。实验证明，TRACE在多个真实数据集上显著提升了恢复精度，为智能城市应用提供了高质量的数据基础。
selection_source: fresh_fetch
motivation: 现实中的GPS轨迹往往因采样率低或覆盖受限而稀疏不全，难以直接用于导航和共享出行等高精度服务。
method: 提出了一种集成记忆机制的状态传播扩散模型（SPDM），通过在去噪步骤间传递状态来增强对难恢复轨迹段的重建能力。
result: "在多个真实数据集上的实验显示，TRACE在不增加显著推理开销的情况下，将轨迹恢复精度提升了26%以上。"
conclusion: TRACE为移动定位服务提供了更可靠的数据支撑，有效推动了数据驱动型城市应用的发展。
---

## 摘要
高质量的 GPS 轨迹对于基于位置的网络服务和智慧城市应用（包括导航、网约车和配送）至关重要。然而，由于数据采集过程中的低采样率和有限的基础设施覆盖，现实世界的轨迹往往是稀疏的，且位置点分布不均。将这些轨迹恢复为密集且连续的形式至关重要，但由于其复杂且不规则的时空模式，这具有很大的挑战性。在本文中，我们介绍了一种名为 TRACE 的新型轨迹恢复扩散模型，它能够从稀疏且不完整的输入中重建密集且连续的轨迹。TRACE 的核心是我们提出的状态传播扩散模型（SPDM），该模型集成了一种新颖的记忆机制，使得在去噪过程中，TRACE 能够保留并利用前几步的中间结果，从而有效地重建那些难以恢复的轨迹段。在多个真实世界数据集上的广泛实验表明，TRACE 的性能优于现有最先进的方法，在没有显著推理开销的情况下，准确率提升了 26% 以上。我们的工作巩固了移动和网络连接位置服务的基础，提升了数据驱动型城市应用的质量和公平性。代码可在以下网址获取：https://github.com/JinmingWang/TRACE
