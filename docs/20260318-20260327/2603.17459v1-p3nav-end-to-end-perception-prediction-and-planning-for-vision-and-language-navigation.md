...
tags: ["query:tp"]
score: 7.0
evidence: 集成感知、预测和规划的导航框架
tldr: 针对视觉语言导航（VLN）中智能体缺乏场景理解能力的问题，本文提出P³Nav框架。该框架将感知、预测与规划集成到统一的端到端流程中，通过提取物体和地图层面的感知线索，并预测未来航点及语义地图，增强了智能体对环境的预判能力。实验证明，该方法在REVERIE、R2R-CE和RxR-CE等多个基准测试上达到了当前最先进的性能。
selection_source: fresh_fetch
motivation: 现有的VLN方法主要关注视觉文本对齐，往往忽视了在规划前进行全面场景理解（感知与预测）的重要性。
method: 提出P³Nav框架，通过融合物体与地图感知、预测未来航点及语义地图，构建了一个综合性的端到端导航规划模块。
result: 该方法在REVERIE、R2R-CE和RxR-CE等主流VLN基准测试中均取得了新的SOTA性能。
conclusion: 将感知、预测与规划深度集成能显著提升VLN智能体的场景理解能力和导航成功率。
---
