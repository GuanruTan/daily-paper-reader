...
title_zh: ENC-Bench：评估多模态大语言模型在电子海图理解能力的基准测试
...
tags: ["query:tp"]
score: 6.0
evidence: 解释海图中的航线约束和几何结构
tldr: "本研究针对电子海图（ENC）理解这一航海安全关键领域，推出了首个专业基准测试集 ENC-Bench。该基准包含从 NOAA 真实数据中提取的 2 万余个样本，涵盖感知、空间推理和决策三大层级。通过对 GPT-4o 等 10 种主流多模态大模型进行评估，发现现有模型在处理海图符号、空间计算及复杂约束推理方面表现欠佳，最高准确率仅为 47.88%，为推动大模型在专业航海领域的应用奠定了基础。"
selection_source: fresh_fetch
motivation: 电子海图对航海安全至关重要，但现有大模型在理解其标准化矢量符号、比例尺渲染及精确几何结构方面的能力尚不明确。
method: "基于 S-57 原始数据构建了包含 20,490 个专家验证样本的 ENC-Bench，通过矢量转图像流水线生成涵盖感知、推理和决策三个层级的测试任务。"
result: "评估显示 10 种顶尖多模态大模型中表现最好的仅达到 47.88% 的准确率，在符号定位、空间计算和多约束推理方面存在系统性挑战。"
conclusion: ENC-Bench 为专业符号推理和安全关键型 AI 研究开辟了新前沿，是推进大模型在专业航海应用中不可或缺的基础设施。
---

## 摘要
电子海图（ENCs）是现代航海安全的关键支柱，但多模态大语言模型（MLLMs）是否能可靠地解释它们仍不清楚。与自然图像或常规图表不同，ENCs 通过标准化的矢量符号、随比例尺变化的渲染以及精确的几何结构来编码法规、水深和航线约束，这需要专业的航海知识进行解读。我们推出了 ENC-Bench，这是首个专门用于专业 ENC 理解的基准测试。ENC-Bench 包含来自 840 张真实美国国家海洋和大气管理局（NOAA）ENC 的 20,490 个经专家验证的样本，组织为三个层级：感知（符号和特征识别）、空间推理（坐标定位、方位、距离）和航海决策（航线合法性、安全评估、多约束下的应急规划）。所有样本均通过校准的矢量转图像流水线从原始 S-57 数据生成，并经过自动一致性检查和专家评审。我们在统一的零样本协议下评估了 10 种最先进的 MLLM，如 GPT-4o、Gemini 2.5、Qwen3-VL、InternVL-3 和 GLM-4.5V。表现最好的模型仅达到 47.88% 的准确率，在符号接地、空间计算、多约束推理以及对光照和比例尺变化的鲁棒性方面面临系统性挑战。通过建立首个严谨的 ENC 基准测试，我们在专业符号推理与安全关键型 AI 的交汇处开辟了新的研究前沿，为推动 MLLM 走向专业航海应用提供了必要的基础设施。
