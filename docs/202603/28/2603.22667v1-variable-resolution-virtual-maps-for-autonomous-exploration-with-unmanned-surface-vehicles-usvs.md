...
title_zh: 面向无人水面艇（USV）自主探索的可变分辨率虚拟地图
...
tags: ["query:tp"]
score: 6.0
evidence: 无人水面艇在海洋环境中的自主探索
tldr: 针对无人艇（USV）在近岸复杂环境下的自主探索，本文提出了一种变分辨率虚拟地图（VRVM）方法。该方法利用自适应四叉树和二元高斯虚拟地标来表示地图不确定性，有效解决了固定分辨率地图在计算和存储上的扩展性难题。通过结合期望最大化（EM）规划器，VRVM能在GNSS受限环境下平衡探索与利用，提高定位可靠性并优化计算资源利用率。
selection_source: fresh_fetch
motivation: 传统的固定分辨率虚拟地图在处理大规模近岸环境时存在计算效率低、存储开销大以及在特征稀疏区域易导致SLAM失效的问题。
method: 提出基于自适应四叉树的变分辨率虚拟地图，通过二元高斯分布建模不确定性，并结合EM规划器进行路径决策。
result: 在VRX Gazebo仿真环境的测试表明，该方法在GNSS退化的近岸场景中表现出更高的安全性和更优的计算资源利用率。
conclusion: VRVM通过动态调整地图分辨率，实现了在复杂水域环境下高效、稳健的自主探索与定位。
---

## 摘要
无人水面艇（USV）在近岸水域的自主探索需要在大范围区域内实现可靠的定位和一致的建图，但这面临着 GNSS 信号衰减、环境引起的定位不确定性以及有限的板载计算能力的挑战。基于虚拟地图的方法通过将因子图 SLAM 与地图不确定性准则紧密耦合，显式地对定位和建图的不确定性进行建模。然而，在固定分辨率的工作空间离散化下，其存储和计算成本的扩展性较差，导致在大规模近岸环境中的效率低下。此外，过度评估特征稀疏的开阔水域可能会因探索与利用之间的不平衡而增加 SLAM 失败的风险。为了解决这些局限性，我们提出了一种可变分辨率虚拟地图（VRVM），这是一种计算高效的方法，通过在自适应四叉树的单元格中放置二元高斯虚拟路标来表示地图不确定性。自适应四叉树实现了面积加权的不确定性表示，在为信息密集区域分配更高分辨率的同时，刻意保持粗糙的远场虚拟路标的不确定性，并降低了地图评估对树局部细化的敏感性。采用期望最大化（EM）规划器，利用 VRVM 评估边界沿线的位姿和地图不确定性，从而平衡探索与利用。我们在 VRX Gazebo 模拟器中，使用真实的码头环境，在探索难度递增的不同测试场景下，将 VRVM 与几种最先进的探索算法进行了评估。结果表明，在 GNSS 信号衰减的近岸环境中，我们的方法提供了更安全的行为和更好的板载计算利用率。
