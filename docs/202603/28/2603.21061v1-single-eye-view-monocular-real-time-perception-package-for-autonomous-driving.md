...
title_zh: 单目视角：面向自动驾驶的单目实时感知软件包
authors: "Haixi Zhang, Aiyinsi Zuo, Zirui Li, Chunshu Wu, Tong Geng, Zhiyao Duan"
date: 2026-03-22
pdf: "https://arxiv.org/pdf/2603.21061v1"
tags: ["query:tp"]
score: 8.0
evidence: 自动驾驶中的目标跟踪与预测
tldr: "针对自动驾驶中摄像头感知技术计算效率不足的问题，本文提出了LRHPerception，一个实时单目感知软件包。该系统结合了端到端学习的高效性与局部建图的丰富细节，集成了目标跟踪、道路分割和深度估计等功能。通过将单目图像处理为包含RGB、分割和深度信息的五通道张量，实现了在单GPU上29 FPS的实时处理速度，比现有最快方法提升了555%。"
selection_source: fresh_fetch
motivation: 现有的基于摄像头的自动驾驶感知技术往往优先考虑效果而忽视了计算效率，难以满足实时性需求。
method: 提出LRHPerception框架，将单目视频处理为包含RGB、道路分割和深度估计的五通道张量，并集成目标检测与轨迹预测。
result: "在单GPU上实现了29 FPS的实时处理速度，相比于最快的基于建图的方法，速度提升了555%。"
conclusion: 该研究证明了通过统一框架整合多种感知任务，可以在保证感知细节的同时显著提升自动驾驶系统的实时运行效率。
---

## 摘要
在基于摄像头的自动驾驶技术飞速发展的同时，有效性往往被优先考虑，而对计算效率的关注较少。为了解决这一问题，本文提出了 LRHPerception，这是一个面向自动驾驶的实时单目感知软件包，利用单视图摄像头视频来理解周围环境。该系统结合了端到端学习的计算效率与局部建图方法丰富的表示细节。通过将目标跟踪与预测、道路分割和深度估计方面的显著改进集成到一个统一框架中，LRHPerception 将单目图像数据处理为包含 RGB、道路分割和像素级深度估计的五通道张量，并辅以目标检测和轨迹预测。实验结果表明，该系统性能强劲，在单个 GPU 上实现了 29 FPS 的实时处理，相比最快的基于建图的方法实现了 555% 的加速。
