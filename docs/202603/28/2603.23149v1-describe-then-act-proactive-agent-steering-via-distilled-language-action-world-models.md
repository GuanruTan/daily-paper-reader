...
title_zh: Describe-Then-Act：通过蒸馏语言-动作世界模型实现主动智能体引导
...
conclusion: 证明了无需昂贵的视觉生成，仅通过蒸馏后的语言-动作世界模型即可实现高效且准确的智能体主动引导。
---

## 摘要
部署安全关键型智能体要求在执行动作之前预见其后果。虽然世界模型为这种主动预见提供了一种范式，但目前依赖视觉模拟的方法会产生极高的延迟，每步通常超过几秒钟。在这项工作中，我们挑战了视觉处理对于预防失败是必不可少的这一假设。我们证明，训练好的策略的潜状态结合其计划动作，已经编码了足够的信息来预测动作结果，从而使视觉模拟在预防失败方面变得多余。为此，我们引入了 DILLO（蒸馏语言-动作世界模型），这是一个快速引导层，将范式从“先模拟后行动”转变为“先描述后行动”。DILLO 通过跨模态蒸馏进行训练，其中特权视觉语言模型教师对离线轨迹进行标注，而以潜状态为条件的大语言模型学生学习预测语义结果。这创建了一条纯文本推理路径，完全绕过了沉重的视觉生成，比基准方法实现了 14 倍的加速。在 MetaWorld 和 LIBERO 上的实验表明，DILLO 能够生成下一状态的高保真描述并引导策略，使任务的平均回合成功率提高了 9.3 个百分点，最高提升达 15 个百分点。
