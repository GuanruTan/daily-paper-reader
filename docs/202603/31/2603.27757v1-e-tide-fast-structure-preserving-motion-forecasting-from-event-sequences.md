...
title_zh: E-TIDE：基于事件序列的快速、结构保持的运动预测
...
tldr: 本研究针对事件相机在高速动态捕捉中的优势，提出了E-TIDE，一种轻量级且端到端可训练的事件张量预测架构。为了解决现有方法计算开销大且依赖大规模预训练的问题，E-TIDE引入了TIDE模块，通过大核混合和活动感知门控机制高效捕捉时空依赖。该方法在保持竞争力的预测性能的同时，显著降低了模型参数量和训练需求，非常适合资源受限环境下的实时部署。
selection_source: fresh_fetch
motivation: 现有的事件预测方法通常依赖沉重的计算骨干和大规模预训练，难以在低延迟和内存受限的实时场景中应用。
method: 提出E-TIDE架构，利用TIDE模块通过大卷积核混合和活动感知门控实现高效的时空交互与特征提取。
result: 在标准数据集上的实验表明，该方法在大幅减少模型规模和训练成本的情况下，达到了与最先进技术相当的预测性能。
conclusion: E-TIDE为资源受限环境下的高速运动预测提供了一种高效、保结构且无需大规模预训练的实用解决方案。
---

## 摘要
事件相机将视觉信息捕获为逐像素亮度变化的异步流，生成稀疏且时间精确的数据。与传统的基于帧的传感器相比，它们在捕获高速动态方面具有显著优势，同时功耗大幅降低。从过去的观测中预测未来的事件表示是一个重要问题，它能够在无需访问未来传感器测量值的情况下，实现诸如未来语义分割或目标跟踪等下游任务。虽然最近的最先进方法取得了强大的性能，但它们通常依赖于计算量巨大的骨干网络，且在某些情况下需要大规模预训练，这限制了它们在资源受限场景中的适用性。在这项工作中，我们介绍了 E-TIDE，这是一种用于事件张量预测的轻量级、端到端可训练架构，旨在无需大规模预训练即可高效运行。我们的方法采用了 TIDE 模块（动态事件的时间交互），其灵感源自针对稀疏事件张量的高效时空交互设计，通过大核混合和活动感知门控来捕获时间依赖性，同时保持较低的计算复杂度。在标准事件数据集上的实验表明，我们的方法在显著减小模型尺寸和降低训练要求的情况下，实现了具有竞争力的性能，使其非常适合在严格的延迟和内存预算下进行实时部署。
