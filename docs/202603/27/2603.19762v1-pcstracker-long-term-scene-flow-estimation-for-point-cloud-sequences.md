...
tags: ["query:tp"]
score: 7.0
evidence: 用于运动分析的时空点轨迹更新模块
tldr: PCSTracker是首个专为点云序列设计的一端到端场景流估计框架，旨在解决长序列中几何演变、遮挡和误差累积导致的运动不一致问题。通过引入迭代几何运动联合优化（IGMO）和时空轨迹更新（STTU）模块，并结合重叠滑动窗口推理策略，该方法在保持32.5 FPS实时性能的同时，显著提升了长序列场景流估计的准确性与时空一致性。
selection_source: fresh_fetch
motivation: 现有场景流估计多局限于两帧设置，难以在长序列中应对几何变化、遮挡及误差累积带来的挑战。
method: 提出包含IGMO模块、STTU模块及重叠滑动窗口推理策略的端到端框架，实现几何运动联合优化与长程一致性维护。
result: 在PointOdyssey3D和ADT3D数据集上达到最优精度，并实现32.5 FPS的实时处理速度。
conclusion: PCSTracker有效解决了长序列点云运动分析中的一致性难题，为3D场景流估计提供了高效且准确的新方案。
---
